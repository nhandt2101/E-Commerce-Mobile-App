...
export default function RegisterSalesman({ navigation }) {
  ...
  const handleLogIn = () => {
    navigation.navigate('LoginSalesman');
  };
  ...
  const handleSignUp = async () => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        'INSERT INTO salesman (email, password) VALUES (?, ?)', [email, password],
        (_, result) => {
    ...
    <Center flex={1}>
      <Box safeArea p={2} w="100%" h="100%" bg="white" rounded="lg" justifyContent="center">
        ...
        <HStack mt="6" justifyContent="center">
            <Text fontSize="md" color="indigo.500" onPress={() => navigation.navigate('Register')}>Đăng ký tài khoản người dùng</Text> 
        </HStack>
